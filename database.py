...
class DatabaseManager:
    ...
    def init_database(self):
        with self.get_connection() as conn:
            ...
            cursor.executescript("""
                ...
                -- Drop the audit log table first to ensure FK constraints are fresh
    ...
    def delete_patient_by_id(self, patient_id, user_id):
        """Deletes a patient and all associated records (visits, services, transactions)."""
        self.logger.info(f"Attempting to delete patient ID: {patient_id} by User ID: {user_id}")
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                ...
                try:
                    # Get visit IDs associated with the patient
                    ...
                    if visit_ids:
                        ...
                        cursor.execute(f"DELETE FROM transactions WHERE visit_id IN ({placeholders})", visit_ids)
                        self.logger.debug(f"Deleted transactions for patient ID {patient_id} (visits: {visit_ids})")
                        
                        # Delete associated visit_services
                        cursor.execute(f"DELETE FROM visit_services WHERE visit_id IN ({placeholders})", visit_ids)
                        self.logger.debug(f"Deleted visit_services for patient ID {patient_id} (visits: {visit_ids})")
                    
                    # Delete associated visits
                    cursor.execute("DELETE FROM visits WHERE patient_id = ?", (patient_id,))
                    self.logger.debug(f"Deleted visits for patient ID {patient_id}")
                    
                    # Finally, delete the patient
                    cursor.execute("DELETE FROM patients WHERE patient_id = ?", (patient_id,))
                    
                    if cursor.rowcount == 0:
                        # If no patient was deleted (e.g., ID didn't exist), rollback
                        conn.rollback()
                    ...
                    conn.commit()
                    self.logger.info(f"Successfully deleted patient ID: {patient_id} and associated data by User ID: {user_id}")
                    self.add_audit_log(user_id, "delete_patient", f"Patient ID: {patient_id}")
                    return True

                except sqlite3.Error as e:
                    # Rollback in case of any error during the transaction
                    conn.rollback()
                    self.logger.exception(f"Error during deletion transaction for patient ID {patient_id}")
                    raise DatabaseOperationError(f"Failed to delete patient and associated data: {str(e)}")
    ...
    def update_patient(self, patient_id, name, phone_number, blood_pressure_systolic=None, blood_pressure_diastolic=None, oxygen_saturation=None, heart_rate=None, user_id=None):
        ...
        try:
            with self.get_connection() as conn:
                ...
                conn.commit()
                self.logger.info(f"Successfully updated patient ID: {patient_id} by User ID: {user_id}")
            ...
            self.logger.exception(f"Error updating patient ID {patient_id}")
            raise DatabaseOperationError(f"Failed to update patient: {str(e)}")

    def get_patient_by_id(self, patient_id):
        """Retrieves patient details by patient_id."""
        self.logger.debug(f"Attempting to retrieve patient details for ID: {patient_id}")
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                ...
                if patient:
                    self.logger.debug(f"Patient found for ID {patient_id}: {patient['name']}")
                    return dict(patient) # Return as a dictionary
                else:
                    self.logger.warning(f"Patient not found for ID: {patient_id}")
                    return None
        except sqlite3.Error as e:
            self.logger.exception(f"Error retrieving patient details for ID {patient_id}")
            raise DatabaseOperationError(f"Failed to retrieve patient details: {str(e)}")
